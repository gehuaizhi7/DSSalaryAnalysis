...
clean_jobT %>%
  ...
  mutate(min_employee = as.numeric(str_extract(low_ppl, "\\d+")),
         max_employee = as.numeric(str_extract(high_ppl, "\\d+")),
         avg_employee= case_when(
           !is.na(min_employee) & is.na(max_employee) ~ min_employee,
           T ~ (min_employee + max_employee) / 2))   %>%
  rename("Min_employee" = min_employee,
        "Max_employee" = max_employee, 
        "Avg_employee" = avg_employee)  %>%
  select(c(-low_ppl,-high_ppl))  -> clean_data  

clean_data %>%
  writeEXCEL("clean_data.xlsx") 
```
...
```{r}
save(state_salary, file = "cleaned.RData")
```
